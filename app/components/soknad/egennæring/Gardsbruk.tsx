        ...
        <VStack gap="5">
...

            {
                ...
                <>
                    ...
                    {
                        skjema.egneGårdsbruk.map(value => <div
                            key={value.organisasjonummer}>{value.organisasjonummer}</div>)
                    }
                    <Button variant={"secondary"} onClick={() => egenGårdsbrukvirksomhetRef.current?.showModal()}>+ Legg
                        til gårdsbruk</Button>
                    <Modal ref={egenGårdsbrukvirksomhetRef} header={{heading: "Legg til gårdsbruk"}} width={600}>
                        <Modal.Body>
                            <form method="dialog" id="egetGårdbruk" onSubmit={() => {
                                leggTilGårdsbruk()
                            }}>
                                <TextField
                                ...
                                <CheckboxGroup legend="Hvilken type gårdsbruk driver du?" onChange={(val: string[]) => setGårdsbruk({
                                    ...gårdsbruk,
                                    typeGårdsbruk: val
                                })}>
                                ...
                                <CheckboxGroup legend="Hvem eier gårdsbruket? Du kan krysse for flere" onChange={(val: string[]) => setGårdsbruk({
                                    ...gårdsbruk,
                                    eierAvGårdsbruk: val
                                })}>
                                    <Checkbox value="dyr">Dyr</Checkbox>
