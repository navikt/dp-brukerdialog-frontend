...
export default function Kvittering() {
  ...
  const mineDagpengerUrl = "https://arbeid.ansatt.dev.nav.no/arbeid/dagpenger/mine-dagpenger-frontend"
  const generellInnsendingUrl = "https://arbeid.ansatt.dev.nav.no/dagpenger/dialog/generell-innsending"
  ...
  return (
    <div className="brukerdialog">
      ...
      <ExternalLink to={mineDagpengerUrl}
                    asButtonVariant={inntektStemmer ? "primary" : "secondary"} className="mt-14">
        Gå til mine dagpenger
      </ExternalLink>

      {!inntektStemmer && (
        <ExternalLink to={generellInnsendingUrl}
                      asButtonVariant="primary" className="mt-14 ml-4">
          Last opp dokumentasjon
        </ExternalLink>
      )}
    </div>
