...
export default function Utdanning() {
    ...
    const [avsluttetUtdanningSiste6MånederFiles, setAvsluttetUtdanningSiste6MånederFiles] = useState<FileObject[]>([]);

    function removeFile(fileToRemove: FileObject) {
        setAvsluttetUtdanningSiste6MånederFiles(avsluttetUtdanningSiste6MånederFiles.filter((file) => file !== fileToRemove));
    }
    const acceptedFiles = avsluttetUtdanningSiste6MånederFiles.filter((file) => !file.error);
    const rejectedFiles = avsluttetUtdanningSiste6MånederFiles.filter((f): f is FileRejected => f.error);
            ...
            <Page className="brukerdialog">
                <VStack gap="6">
...

                    {
                        ...
                        <>
                            <Alert variant="info">
                                ...
                                avsluttende eksamen eller aktivitet. Hvis du har avbrutt skolegangen, kan du legge ved
                                bekreftelse fra skolen på dette.
                            </Alert>
                            <RadioGroup
                                legend="Ønsker du å dokumentere dette nå?"
                                value={utdanning.dokumenterAvsluttetUtdanningSiste6MånederNå}
                                onChange={(value: boolean) => {
                                    setUtdanning({...utdanning, dokumenterAvsluttetUtdanningSiste6MånederNå: value})
                                }}
                            >
                            ...
                            {utdanning.dokumenterAvsluttetUtdanningSiste6MånederNå === true &&
                                <VStack gap="6">
                                    <FileUpload.Dropzone
                                        ...
                                        onSelect={(newFiles) => setAvsluttetUtdanningSiste6MånederFiles([...avsluttetUtdanningSiste6MånederFiles, ...newFiles])}
                                    />
                    ...
                    <JaNeiFaktum
                        ...
                        vedEndring={(value: boolean) => {
                            setUtdanning({...utdanning, starteEllerFullføreUtdanningSamtidigSomMottarDagpenger: value})
                        }}
                        aktiv={utdanning.tarUtdanningEllerOpplæring === false}
                    />

                    {
                        (utdanning.tarUtdanningEllerOpplæring === true || utdanning.starteEllerFullføreUtdanningSamtidigSomMottarDagpenger === true) &&
                        <Alert variant="warning">
                            <VStack gap="5">
                                <div>
                                    For å få innvilget dagpenger mens du tar utdanning eller opplæring, må du sende
                                    inn <a href="https://www.nav.no/fyllut/nav040605">Søknad om å beholde dagpengene
                                    mens du tar utdanning eller opplæring - NAV 04-06.05</a>, i tillegg til å sende inn
                                    denne søknaden om dagpenger.
                                </div>
                                <div>
